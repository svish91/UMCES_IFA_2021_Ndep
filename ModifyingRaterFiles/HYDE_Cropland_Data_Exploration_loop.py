...
for idx_yr in range(0,len(file_names1)):
...
for idx_yr in range(0,len(file_names1)):
    ...
    # changing resolution
    lat = np.arange(0, Crplnd_mat.shape[0], 15)
    lon = np.arange(0, Crplnd_mat.shape[1], 30)
    ...
    new_dataset = rasterio.open("Cropland{0}_HYDE_Zhou.tif".format(file_names1[idx_yr]), 'w',  driver='GTiff',height = outMatrix.shape[0], width = outMatrix.shape[1],
                                count=1, dtype=str(outMatrix.dtype),
                                crs='+init=epsg:4326',#'+proj=utm +ellps=WGS84 +datum=WGS84  +units=degrees +no_defs',
                                transform=transform)
