...
# Merging two files
yrs_to_consider = np.arange(2019,2051,1)
...
NOx_2019_2050_concat = NOx_concat_tmp[idx_yrs_to_consider,:,:]
# NHy
...
NHy_2019_2050_concat = NHy_concat_tmp[idx_yrs_to_consider,:,:]
# changing units
#units: g N m - 2 yr - 1 = 1000 kg N km-2 yr-1
NOx_2019_2050_concat_kgNkm = 1000*NOx_2019_2050_concat
NHy_2019_2050_concat_kgNkm = 1000*NHy_2019_2050_concat
...
for yr1 in range(0,len(yrs_to_consider)):
    ...
    new_dataset = rasterio.open("NOx_yr{0}.tif".format(np.int(yrs_to_consider[yr1])), 'w',  driver='GTiff',
                                height = NOx_2019_2050_concat_kgNkm.shape[1], width = NOx_2019_2050_concat_kgNkm.shape[2],
                                            count=1, dtype=str(NOx_2019_2050_concat_kgNkm.dtype),
                                            crs='+init=epsg:4326',#'+proj=utm +ellps=WGS84 +datum=WGS84  +units=degrees +no_defs',
                                            transform=transform)
    outMatrix = np.reshape(NOx_2019_2050_concat_kgNkm[yr1,:,:],(NOx_2019_2050_concat_kgNkm.shape[1],NOx_2019_2050_concat_kgNkm.shape[2]))
    ...
    new_dataset = rasterio.open("NHy_yr{0}.tif".format(np.int(yrs_to_consider[yr1])), 'w',  driver='GTiff',
                                height = NHy_2019_2050_concat_kgNkm.shape[1], width = NHy_2019_2050_concat_kgNkm.shape[2],
                                            count=1, dtype=str(NHy_2019_2050_concat_kgNkm.dtype),
                                            crs='+init=epsg:4326',#'+proj=utm +ellps=WGS84 +datum=WGS84  +units=degrees +no_defs',
                                            transform=transform)
    outMatrix = np.reshape(NHy_2019_2050_concat_kgNkm[yr1,:,:],(NHy_2019_2050_concat_kgNkm.shape[1],NHy_2019_2050_concat_kgNkm.shape[2]))
    new_dataset.write(outMatrix,1)
    new_dataset.close()
