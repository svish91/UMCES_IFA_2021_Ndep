...
for yr in np.arange(1961,2020,1):
    ...
    crfile_dir = r'.\Modified_Raster_Files\Cropland_HYDE'
...
for co in range(0,len(shapefile['CNTRY_NAME'])):
    ...
    plt.ylim([0,20])
    plt.savefig(r".\Time_Series_Co_2000_2017\TotNdep_{0}_kgNha.png".format(shapefile['CNTRY_NAME'][co]), dpi=200)
...
for yr_to_plot in np.arange(1961,2019,1):
    ...
    #(missing values \nare plotted in grey)
    plt.savefig(r".\Results\ACCMIP\HYDE\Yearly_Maps_1961_2018\Ndep_{0}_kgNha_m.png".format(yr_to_plot), dpi=200)
