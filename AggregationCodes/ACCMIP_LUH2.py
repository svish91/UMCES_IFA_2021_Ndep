...
df_NOx_kgNha = pd.DataFrame(0, index=np.arange(len(shapefile)), columns=np.arange(1961,2020,1))
df_NOx_kgNha = df_NOx_kgNha.rename(index = shapefile['CNTRY_NAME'])
df_NHy_kgNha= pd.DataFrame(0, index=np.arange(len(shapefile)), columns=np.arange(1961,2020,1))
df_NHy_kgNha = df_NHy_kgNha.rename(index = shapefile['CNTRY_NAME'])
df_totNdep_kgNha= pd.DataFrame(0, index=np.arange(len(shapefile)), columns=np.arange(1961,2020,1))
df_totNdep_kgNha = df_totNdep_kgNha.rename(index = shapefile['CNTRY_NAME'])
df_cropland_km2= pd.DataFrame(0, index=np.arange(len(shapefile)), columns=np.arange(1961,2020,1))
df_cropland_km2 = df_cropland_km2.rename(index = shapefile['CNTRY_NAME'])
# NOx
for yr in np.arange(1961,2020,1):
    print(yr)
    if yr > 2015:
        yrcr = 2015
    else:
...
for co in range(0,len(shapefile['CNTRY_NAME'])):
    ...
    plt.ylim([0,20])
    plt.savefig(r".\Results\LUH2\Time_Series_Co_1961_2020\TotNdep_{0}_kgNha.png".format(shapefile['CNTRY_NAME'][co]), dpi=200)
...
for yr_to_plot in np.arange(1961,2020,1):
    plt.clf()
    ...
    #(missing values \nare plotted in grey)
    plt.savefig(r".\Results\ACCMIP\LUH2\Yearly_maps_1961_2020_SameLim\Ndep_{0}_kgNha_m.png".format(yr_to_plot), dpi=200)
