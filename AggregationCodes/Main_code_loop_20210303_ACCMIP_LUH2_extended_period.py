...
for yr_to_plot in np.arange(2019,2051,1):
    ...
    #(missing values \nare plotted in grey)
    plt.savefig(r".\Results\Yearly_Maps_2019_2050\Nox\NOx_{0}_kgNha_m.png".format(yr_to_plot), dpi=200)
    plt.close()
...
for yr_to_plot in np.arange(2019,2051,1):
    ...
    ax.set_title("NOx deposition (kgN/ha) {0} ".format(yr_to_plot))
    #(missing values \nare plotted in grey)
    plt.savefig(r".\Results\Yearly_Maps_2019_2050\NHy\NHy_{0}_kgNha_m.png".format(yr_to_plot), dpi=200)
    plt.close()
