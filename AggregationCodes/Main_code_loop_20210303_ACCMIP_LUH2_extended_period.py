...
for yr in np.arange(1961,2051,1):
    ...
    file_dir = r'G:\My Drive\N_deposition_project\Gridded DATA\Data_Organization\Modified_Raster_Files\Ndep_ACCMIP'
    file_name = "NOx_yr{0}".format(yr)
    crfile_dir = r'G:\My Drive\N_deposition_project\Gridded DATA\Data_Organization\Modified_Raster_Files\Cropland_LUH2'
...
for co in range(0,len(shapefile['CNTRY_NAME'])):
    ...
    plt.ylim([0,20])
    plt.savefig(r"G:\My Drive\N_deposition_project\Gridded DATA\Data_Organization\Results\ACCMIP\LUH2\Time_Series_1961_2050\TotNdep_{0}_kgNha.png".format(shapefile['CNTRY_NAME'][co]), dpi=200)
...
for yr_to_plot in np.arange(2019,2051,1):
    ...
    #(missing values \nare plotted in grey)
    plt.savefig(r"D:\GoogleDrive\N_deposition_project\Gridded DATA\Data_Organization\Results\Yearly_Maps_2019_2050\TotalNdep\Ndep_{0}_kgNha_m.png".format(yr_to_plot), dpi=200)
...
for yr_to_plot in np.arange(2019,2051,1):
    ...
    #(missing values \nare plotted in grey)
    plt.savefig(r"D:\GoogleDrive\N_deposition_project\Gridded DATA\Data_Organization\Results\Yearly_Maps_2019_2050\Nox\NOx_{0}_kgNha_m.png".format(yr_to_plot), dpi=200)
    plt.close()
...
for yr_to_plot in np.arange(2019,2051,1):
    ...
    ax.set_title("NOx deposition (kgN/ha) {0} ".format(yr_to_plot))
    #(missing values \nare plotted in grey)
    plt.savefig(r"D:\GoogleDrive\N_deposition_project\Gridded DATA\Data_Organization\Results\Yearly_Maps_2019_2050\NHy\NHy_{0}_kgNha_m.png".format(yr_to_plot), dpi=200)
    plt.close()
