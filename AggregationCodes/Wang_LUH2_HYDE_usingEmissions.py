...
for yr in yrN:
    ...
    if (yr == 1970)  or (yr == 1980)  or (yr == 1990) or (yr == 1997):
        ...
        file_dir = r'.\N_emission_data_using_textfile\Raster_file_Zhou_extended_period'
    ...
    crfile_dir = r'.n\Modified_Raster_Files\Cropland_LUH2\For_Zhou\Modified'
...
for co in range(0,len(shapefile['CNTRY_NAME'])):
    ...
    plt.ylim([0,40])
    plt.savefig(r".\N_emission_data_using_textfile\Complete_TS\New_approach\TotNdep_{0}_kgNha_EP_Na.png".format(shapefile['CNTRY_NAME'][co]), dpi=200)
...
for co in range(0,len(shapefile['CNTRY_NAME'])):
    ...
    plt.title(shapefile['CNTRY_NAME'][co])
    plt.ylim([0,40])
    plt.savefig(r".\N_emission_data_using_textfile\Complete_TS\New_appraoch_Zhou_HYDE\TotNdep_{0}_kgNha_EP_Na.png".format(shapefile['CNTRY_NAME'][co]), dpi=200)
